...
const SignIn = (props) => {
  ...
  const [dataForm, setDataForm] = useState(dataSignIn);
  const [accept, setAccept] = useState(false);
  const [sex, setSex] = useState("true");
  const [dateValue, setDateValue] = useState(null);
  const [showCalendar, setShowCalendar] = useState(false);
  const [checkFillDate, setCheckFillDate] = useState(false);
  ...
  return (
    <section className="w-screen flex items-center justify-center h-screen overflow-auto inset-0 z-40 fixed bg-bg-overlay">
      <div
